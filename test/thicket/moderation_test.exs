defmodule Thicket.ModerationTest do
  ...
  setup do
    ...
    {:ok, channel} =
      Identity.create_channel(user, %{
        handle: "mod#{System.unique_integer([:positive])}",
        display_name: "Mod"
      })

    {:ok, target} =
      Identity.create_channel(other, %{
        handle: "target#{System.unique_integer([:positive])}",
        display_name: "Target"
      })
  ...
  test "reports require an administrator to resolve and create an audit event", %{
    user: user,
    channel: channel,
    target: target
  } do
