defmodule Thicket.IdentityMilestoneTest do
  ...
  test "operator invitations are hashed and atomically create the user and first channel" do
    ...
    attrs = %{email: "new@example.com", invite_code: code, handle: "Garden_Path", display_name: "Garden Path"}
  ...
  test "expired, revoked, and fully redeemed invitations are rejected" do
    ...
    assert {:error, %Ecto.Changeset{}} = Identity.register_invited_user(%{attrs | email: "two@example.com", handle: "two"})

    invitation |> Ecto.Changeset.change(redemption_count: 0, revoked_at: DateTime.utc_now(:second)) |> Repo.update!()
    assert {:error, %Ecto.Changeset{}} = Identity.register_invited_user(%{attrs | email: "three@example.com", handle: "three"})

    expired = invitation |> Ecto.Changeset.change(revoked_at: nil, expires_at: DateTime.add(DateTime.utc_now(:second), -1, :second)) |> Repo.update!()
  ...
  test "channel authorization rejects another user's channel" do
    ...
    assert {:ok, channel} = Identity.create_channel(owner, %{handle: "owner", display_name: "Owner"})
    assert_raise Ecto.NoResultsError, fn -> Identity.get_channel_for_user!(outsider, channel.id) end
  end
end
