defmodule Thicket.Social do
  ...
  def list_home_posts(%Channel{id: channel_id}, limit \\ 40) do
    followed_ids = from(f in Follow, where: f.follower_channel_id == ^channel_id, select: f.followed_channel_id)
  ...
  def follow(%Channel{} = follower, %Channel{} = followed) do
    Multi.new()
    |> Multi.insert(:follow, %Follow{follower_channel_id: follower.id, followed_channel_id: followed.id} |> Follow.changeset(%{}))
  ...
  def unfollow(%Channel{} = follower, %Channel{} = followed) do
    {count, _} = Repo.delete_all(from f in Follow, where: f.follower_channel_id == ^follower.id and f.followed_channel_id == ^followed.id)
    {:ok, count > 0}
  end
  ...
  def create_comment(%Channel{} = channel, %Post{comments_locked: false} = post, attrs) do
    ...
    with :ok <- validate_parent(post.id, parent_id),
         {:ok, html, _version} <- Rendering.render_comment(Map.get(attrs, "source") || Map.get(attrs, :source) || "") do
      comment = %Comment{post_id: post.id, parent_id: parent_id, channel_id: channel.id, rendered_html: html}
      ...
      |> Multi.run(:notification, fn repo, %{comment: comment} ->
        ...
        insert_notification(repo, post.channel_id, channel.id, kind, "comment", comment.id)
      end)
      |> Repo.transaction()
  ...
  def create_comment(%Channel{}, %Post{comments_locked: true}, _attrs), do: {:error, :comments_locked}
  ...
  defp render_post(changeset) do
    ...
    if changeset.valid? do
      case Rendering.render(source, format) do
        ...
        {:error, _reason} -> Ecto.Changeset.add_error(changeset, :source, "could not be rendered")
      end
    else
      changeset
    end
  ...
  defp maybe_publish(changeset) do
    if Ecto.Changeset.get_field(changeset, :state) == :published and is_nil(Ecto.Changeset.get_field(changeset, :published_at)) do
      Ecto.Changeset.put_change(changeset, :published_at, DateTime.utc_now(:second))
    else
      changeset
    end
  end
  ...
  defp react(schema, channel, post, kind) do
    reaction = struct(schema, channel_id: channel.id, post_id: post.id)

    Multi.new()
    |> Multi.insert(:reaction, reaction, on_conflict: :nothing, conflict_target: [:channel_id, :post_id])
    |> maybe_notify(post.channel_id, channel.id, kind, "post", post.id)
    |> Repo.transaction()
    |> unwrap_multi(:reaction)
  end

  defp unreact(schema, channel, post) do
    {count, _} = Repo.delete_all(from r in schema, where: r.channel_id == ^channel.id and r.post_id == ^post.id)
    {:ok, count > 0}
  end
  ...
  defp validate_parent(post_id, parent_id) do
    if Repo.exists?(from c in Comment, where: c.id == ^parent_id and c.post_id == ^post_id), do: :ok, else: {:error, :invalid_parent}
  end
