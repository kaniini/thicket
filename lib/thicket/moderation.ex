defmodule Thicket.Moderation do
  ...
  def unblock(%Channel{} = blocker, %Channel{} = blocked) do
    Repo.delete_all(
      from b in Block,
        where: b.blocker_channel_id == ^blocker.id and b.blocked_channel_id == ^blocked.id
    )
  ...
  def blocked?(%Channel{id: blocker_id}, %Channel{id: blocked_id}) do
    Repo.exists?(
      from b in Block,
        where: b.blocker_channel_id == ^blocker_id and b.blocked_channel_id == ^blocked_id
    )
  end

  def blocked_between?(%Channel{id: left_id}, %Channel{id: right_id}) do
    blocked_between_ids?(left_id, right_id)
  end

  def blocked_between_ids?(left_id, right_id) when is_binary(left_id) and is_binary(right_id) do
    Repo.exists?(
      from b in Block,
        where:
          (b.blocker_channel_id == ^left_id and b.blocked_channel_id == ^right_id) or
            (b.blocker_channel_id == ^right_id and b.blocked_channel_id == ^left_id)
    )
  end

  def blocked_channel_ids(%Channel{id: channel_id}) do
    from(b in Block,
      where: b.blocker_channel_id == ^channel_id or b.blocked_channel_id == ^channel_id,
      select: {b.blocker_channel_id, b.blocked_channel_id}
    )
    |> Repo.all()
    |> Enum.map(fn
      {^channel_id, blocked_id} -> blocked_id
      {blocker_id, ^channel_id} -> blocker_id
    end)
  end
  ...
  def list_open_reports(%User{admin: true}) do
    Report
    |> where([r], r.state == :open)
    |> order_by(asc: :inserted_at)
    |> preload(:reporter_channel)
    |> Repo.all()
  end
  ...
  def resolve_report(%User{admin: true} = admin, %Report{} = report, state)
    ...
    |> Multi.update(
      :report,
      Ecto.Changeset.change(report, state: state, resolved_by_id: admin.id, resolved_at: now)
    )
    |> Multi.insert(:audit, %AuditEvent{
  ...
  def put_domain_rule(%User{admin: true} = admin, attrs) do
    ...
    |> Multi.insert(:audit, fn %{rule: rule} ->
      Ecto.Changeset.change(%AuditEvent{}, %{
        actor_user_id: admin.id,
        action: "domain.suspend",
        subject_type: "domain_rule",
        subject_id: rule.id
      })
    end)
    |> Repo.transaction()
  ...
  def list_domain_rules(%User{admin: true}),
