  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <div id="channels" phx-update="stream" class="mb-10 grid gap-4 sm:grid-cols-2">
        ...
        >
          <h2 class="text-xl font-bold">{channel.display_name}</h2>
          <p class="text-slate-500">@{channel.handle}</p>
          <div class="mt-3 flex gap-4">
            <.link navigate={~p"/channels/#{channel.handle}/settings"} class="text-emerald-700">
              Edit channel
            </.link>
            <.form
              :if={@current_scope.channel.id != channel.id}
              for={%{}}
              action={~p"/channels/switch"}
              method="post"
            >
            ...
            <span :if={@current_scope.channel.id == channel.id} class="text-slate-400">Active</span>
          </div>
        </article>
      </div>
      <h2 class="mb-4 text-2xl font-bold">Create another channel</h2>
      <.form for={@form} id="channel-form" phx-submit="create" class="max-w-lg space-y-4">
        <.input field={@form[:handle]} label="Handle" required /><.input
